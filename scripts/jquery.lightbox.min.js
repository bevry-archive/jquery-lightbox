...
a("#lightbox-caption-title").click(function(){window.open(a(this).attr("href"));return false});a("#lightbox-overlay, #lightbox, #lightbox-loading-link, #lightbox-btnClose").unbind().click(function(){a.Lightbox.finish();return false});this.auto_relify&&this.relify();return true},relify:function(){var b={},c=0,d=this.rel;a.each(a("[rel*="+d+"]"),function(g,f){var i=a(f).attr("rel");if(i===d)i=c;if(typeof b[i]==="undefined"){b[i]=[];c++}b[i].push(f)});a.each(b,function(g,f){a(f).lightbox()});return true},
init:function(b,c){if(typeof c==="undefined"){c=b;b=0}this.images.clear();this.images.create(c);if(this.images.isEmpty()){a.console.warn("WARNING","Lightbox started, but no images: ",b,c);return false}if(!this.images.current(b)){a.console.warn("WARNING","Could not find current image: ",b,this.images);return false}return true},start:function(){this.visible=true;this.scroll==="disable"&&a(document.body).css("overflow","hidden");a("embed, object, select").css("visibility","hidden");this.resizeBoxes("general");
