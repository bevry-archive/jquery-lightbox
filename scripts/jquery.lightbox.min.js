...
var d=c;b=b||{};var f=function(e,h){if(typeof e==="object")for(var j in e)e[j]=f(e[j],h);else if(typeof h==="array"){j=0;for(var k=e.length;j<k;++j)e[j]=f(e[j],h)}else e=h+e;return e};if(c&&typeof b.files==="undefined"){this.compressed=null;var g=a("script[src*="+this.files.compressed.scripts.lightbox+"]:first");if(g.length!==0){a.extend(true,this.files,this.files.compressed);this.compressed=true}else{g=a("script[src*="+this.files.uncompressed.scripts.lightbox+"]:first");if(g.length!==0){a.extend(true,
this.files,this.files.uncompressed);this.compressed=false}}if(this.compressed===null){window.console.error("Lightbox was not able to find it's javascript script tag necessary for auto-inclusion.");d=false}else{this.src=g.attr("src");this.baseurl=this.src.substring(0,this.src.indexOf(this.files.scripts.lightbox));this.files=f(this.files,this.baseurl);b=a.extend(b,a.params_to_json(this.src))}this.images.image={src:"",title:"Untitled",description:"",name:"",color:null,width:null,height:null,id:null,
image:true};this.images.prepare=function(e){var h=a.extend({},this.image);if(e.tagName){e=a(e);if(e.attr("src")||e.attr("href")){h.src=e.attr("src")||e.attr("href");h.title=e.attr("title")||e.attr("alt")||h.title;h.name=e.attr("name")||"";h.color=e.css("backgroundColor");e=h.title.indexOf(": ");if(e>0){h.description=h.title.substring(e+2)||h.description;h.title=h.title.substring(0,e)||h.title}}else h=null}else h=e.src?a.extend(this.image,e):null;if(h)h.id=h.id||h.src+h.title+h.description;return h};
this.images.create=function(e){var h=this;if(e.each)e.each(function(k,l){h.create(l)});else{var j=h.prepare(e);j?h.push(j):window.console.error("We dont know what we have:",e,j);return h}}}else if(typeof b.files==="object")b.files=f(b.files,this.baseurl);else d=false;for(var i in this.options){g=this.options[i];if(typeof b[g]==="object"&&typeof this[g]==="object")this[g]=a.extend(true,this[g],b[g]);else if(typeof b[g]!=="undefined")this[g]=b[g]}delete i;this.ie6=c&&navigator.userAgent.indexOf("MSIE 6")>=
...
this.text.closeInfo+'">'+this.text.close+'</a></span></div><div id="lightbox-infoContainer-clear"></div></div></div></div>');this.resizeBoxes();this.repositionBoxes();a("#lightbox,#lightbox-overlay,#lightbox-overlay-text-interact").hide();this.ie6&&this.ie6_support&&a("#lightbox-overlay").css({position:"absolute",top:"0px",left:"0px"});a.each(this.files.images,function(){var g=new Image;g.onload=function(){g=g.onload=null};g.src=this});a(window).unbind("resize").resize(function(){a.Lightbox.resizeBoxes("resized")});
...
c=d.width;d=d.height;e=a(window).width();var f=a(window).height();if(this.auto_resize!==false){e=Math.floor(e*0.8);f=Math.floor(f*0.8);for(var g;c>e||d>f;){if(c>e){g=e/c;c=e;d=Math.floor(d*g)}if(d>f){g=f/d;d=f;c=Math.floor(c*g)}}}var i=a("#lightbox-imageBox").width();g=a("#lightbox-imageBox").height();f=c+this.padding*2;var e=d+this.padding*2;i=i-f;g=g-e;a("#lightbox-nav-btnPrev,#lightbox-nav-btnNext").css("height",e);a("#lightbox-infoBox").css("width",f);if(b==="transition")if(i===0&&g===0){this.pause(this.speed/
3);this.showImage(null,3)}else{a("#lightbox-image").width(c).height(d);a("#lightbox-imageBox").animate({width:f,height:e},this.speed,function(){a.Lightbox.showImage(null,3)})}else{a("#lightbox-image").animate({width:c,height:d},this.speed);a("#lightbox-imageBox").animate({width:f,height:e},this.speed)}this.repositionBoxes({nHeight:e,speed:this.speed});return true},repositioning:false,reposition_failsafe:false,repositionBoxes:function(b){if(this.repositioning){this.reposition_failsafe=true;return null}this.repositioning=
...
break;case 3:a("#lightbox-loading").hide();a("#lightbox-image").fadeIn(this.speed*1.5,function(){a.Lightbox.showImage(null,4)});this.preloadNeighbours();this.handlers.show!==null&&this.handlers.show(b);break;case 4:var f=a("#lightbox-caption-title").html(b.title||"Untitled");if(this.download_link)f.attr("href",this.download_link?b.src:"");delete f;a("#lightbox-caption-seperator").html(b.description?": ":"");a("#lightbox-caption-description").html(b.description||"&nbsp;");this.images.length>1?a("#lightbox-currentNumber").html(this.text.image+
